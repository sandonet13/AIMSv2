...
$(function () {
  "use strict";
  $.post(admin_url + 'mention/get_staff_mentions').done(function(response) {
    ...
    data_mention_config = {
      ...
      insertTpl: '<a href="' + admin_url+'staff/profile/${id}" data-staff-id="data_staff_id_${id}">${name}</a>',
  ...
  $.post(admin_url + 'mention/get_object_mentions').done(function(response) {
      response = JSON.parse(response);
...
function post_inputor(){
  ...
  $.post(admin_url + 'mention/add_post', data).done(function(response) {
        response = JSON.parse(response);
...
function appendData() {
  "use strict";
  $.post(admin_url + 'mention/load_mention_ajax/0/'+counter).done(function(response) {
      response = JSON.parse(response);
...
function add_comment_isn(id) {
  ...
  var url = '';
  $.post(admin_url + 'mention/add_comment', {
      content: content,
      postid: id,
      url: url,
...
function remove_post_comment_isn(id, postid) {
  "use strict";
  requestGetJSON('mention/remove_post_comment/' + id + '/' + postid).done(function(response) {
      if (response.success === true || response.success == 'true') {
...
function delete_post_isn(postid) {
  "use strict";
  if (confirm_delete()) {
      $.post(admin_url + 'mention/delete_post/' + postid, function(response) {
...
function refresh_post_comments_isn(postid) {
  "use strict";
  $.post(admin_url + 'mention/init_post_comments/' + postid + '?refresh_post_comments=true').done(function(response) {
      $('[data-comments-postid="' + postid + '"]').append(response);
  });
}

//see details
function see_details_isn(id) {
  "use strict";
  requestGet('mention/get_post_data/'+id).done(function(response) {
    if (typeof($(this).data('close')) == 'undefined') {
          $('#show_post_detail').modal('show');
          $('#show_post_detail div.modal-content').html(response);
