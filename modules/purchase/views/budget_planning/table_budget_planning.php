...
$sIndexColumn = 'id';
$sTable       = db_prefix().'budget_planning';
$join         = [ 'LEFT JOIN '.db_prefix().'departments ON '.db_prefix().'departments.departmentid = '.db_prefix().'budget_planning.department' ];
$where = [];
...
if(isset($project)){
    array_push($where, ' AND '.db_prefix().'budget_planning.project = '.$project);
}
...
foreach ($rResult as $aRow) {
   ...
   for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if($aColumns[$i] == 'request_date'){
            ...
            $name = '<a href="' . admin_url('purchase/view_budget_planning/' . $aRow['id'] ).'">'.$aRow['pur_rq_name'] . '</a>';

            $name .= '<div class="row-options">';

            $name .= '<a href="' . admin_url('purchase/view_budget_planning/' . $aRow['id'] ).'" >' . _l('view') . '</a>';

            if ( (has_permission('purchase_request', '', 'edit') || is_admin()) &&  $aRow['status'] != 2) {
                $name .= ' | <a href="' . admin_url('purchase/budget_planning/' . $aRow['id'] ).'" >' . _l('edit') . '</a>';
            }

            if (has_permission('purchase_request', '', 'delete') || is_admin()) {
                $name .= ' | <a href="' . admin_url('purchase/delete_budget_planning/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
            }
            ...
            $_data = '<a href="' . admin_url('purchase/view_budget_planning/' . $aRow['id'] ).'">'.$aRow['pur_rq_code'] . '</a>';
        }elseif($aColumns[$i] == 'project'){

            $_data = get_po_html_by_budget_planning($aRow['id']);
        }
