 ...
 **/
class fs_elFinderVolumeFTP extends fs_elFinderVolumeDriver
{
    ...
    private function deleteDir($dirPath)
    {
        if (!is_dir($dirPath)) {
            ...
            $success = true;
            foreach (array_reverse(fs_elFinderVolumeFTP::listFilesInDirectory($dirPath, false)) as $path) {
        ...
        foreach ($files as $file) {
            if (!in_array($file, $excludes)) {
                ...
                if (is_link($path)) {
                    ...
                    $subs = fs_elFinderVolumeFTP::listFilesInDirectory($path, $omitSymlinks, $prefix . $file . DIRECTORY_SEPARATOR);
                    if ($subs) {
