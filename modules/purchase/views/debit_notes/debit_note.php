      ...
      <div class="row">
       ...
       <label for="vendorid"><?php echo _l('vendor'); ?></label>
        <select name="vendorid" id="vendorid" class="selectpicker" data-live-search="true" data-width="100%" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>" >
            ...
            <option value="<?php echo html_entity_decode($s['userid']); ?>" <?php if(isset($debit_note) && $debit_note->vendorid == $s['userid']){ echo 'selected'; }else{ if(isset($ven) && $ven == $s['userid']){ echo 'selected';} } ?>><?php echo html_entity_decode($s['company']); ?></option>
              <?php } ?>
        </select> 
      </div>
 <div class="row">
