...
$fn_get_data = function () {
  ...
  if (!is_client_logged_in()) {
    ...
    if (!$has_permission) {
      $this->db->where('project_id IN (SELECT id FROM ' . db_prefix() . 'pur_activity_log WHERE staffid=' . get_staff_user_id() . ')');
    }
  }
  if (is_client_logged_in()) {
  ...
  $activities = $this->db->get(db_prefix() . 'pur_activity_log')->result_array();
  $i          = 0;
  foreach ($activities as $activity) {
    $seconds          = get_string_between($activity['date'], '<seconds>', '</seconds>');
    $other_lang_keys  = get_string_between($activity['date'], '<lang>', '</lang>');
    $_additional_data = $activity['rel_type'];
    ...
    if (strpos($_additional_data, 'project_status_') !== false) {
      $_additional_data = get_list_approval_details($activity['staffid'],'pur_request');
        ...
        foreach ($widget_data as $activity) {
          ...
          <div class="feed-item">
            ...
            <div class="text">
              ...
              <?php echo _l('User'); ?>: <a href="<?php echo admin_url('projects/view/' . $activity['project_id']); ?>"><?php echo $activity['project_name']; ?></a>
            </div>
