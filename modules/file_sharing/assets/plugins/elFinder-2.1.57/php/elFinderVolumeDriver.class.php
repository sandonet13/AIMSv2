 ...
 */
abstract class fs_elFinderVolumeDriver
{
    ...
    public function mount(array $opts)
    {
        ...
        if ($this->mimeDetect === 'internal' && !fs_elFinderVolumeDriver::$mimetypesLoaded) {
            fs_elFinderVolumeDriver::loadMimeTypes(!empty($this->options['mimefile']) ? $this->options['mimefile'] : '');
        }
    ...
    /**
     * Return Extention/MIME Table (fs_elFinderVolumeDriver::$mimetypes)
     *
     * @return array
     * @author Naoki Sawada
     */
    ...
    {
        // load mime.types
        if (!fs_elFinderVolumeDriver::$mimetypesLoaded) {
            fs_elFinderVolumeDriver::loadMimeTypes();
        }
        return fs_elFinderVolumeDriver::$mimetypes;
    }
    ...
    protected function mimetype($path, $name = '', $size = null, $mime = null)
    {
        ...
        if (!$nameCheck && is_readable($path) && $size > 0) {
            ...
            if ($type) {
                ...
                if ($ext && preg_match('~^application/(?:octet-stream|(?:x-)?zip|xml)$~', $type)) {
                    ...
                    if (isset(fs_elFinderVolumeDriver::$mimetypes[$ext])) {
                        $type = fs_elFinderVolumeDriver::$mimetypes[$ext];
                    }
        ...
        if (!$type) {
            ...
            $type = fs_elFinderVolumeDriver::mimetypeInternalDetect($name);
            if ($type === 'unknown') {
                if ($mime) {
                    $type = $mime;
                } else {
    ...
    {
        if (!fs_elFinderVolumeDriver::$mimetypesLoaded) {
            fs_elFinderVolumeDriver::$mimetypesLoaded = true;
            $file = false;
    ...
    static protected function mimetypeInternalDetect($path = '')
    {
        ...
        return ($ext && isset(fs_elFinderVolumeDriver::$mimetypes[$ext])) ? fs_elFinderVolumeDriver::$mimetypes[$ext] : 'unknown';
    }
    ...
    protected function checkExtractItems($path, $checks = null)
    {
        ...
        foreach ($files as $name) {
            ...
            if ($isDir) {
                ...
                if ($chkMime && ($mimeByName = fs_elFinderVolumeDriver::mimetypeInternalDetect($name)) && !$this->allowPutMime($mimeByName)) {
                    self::localRmdirRecursive($p);
                    $res['mimes'][] = $p;
                    $res['rmNames'][] = $utf8Name;
                    continue;
