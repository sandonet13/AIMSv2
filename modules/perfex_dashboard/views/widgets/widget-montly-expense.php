...
$fn_get_data = function () {
  ...
  // echo $sql_expense;
$sql_expense_pembiayaan = "
SELECT MONTH(tblpur_request.request_date) AS month, SUM(tblpur_request_detail.total) AS total_amount FROM `tblpur_request` LEFT JOIN `tblpur_request_detail` ON tblpur_request.id = tblpur_request_detail.pur_request where purchase_type = 'Pembiayaan' AND status = 2
";
$sql_expense_jasa = "
SELECT MONTH(tblpur_request.request_date) AS month, SUM(tblpur_request_detail.total) AS total_amount FROM `tblpur_request` LEFT JOIN `tblpur_request_detail` ON tblpur_request.id = tblpur_request_detail.pur_request where purchase_type = 'Jasa' AND status = 2
";
$sql_expense_barang = "
SELECT MONTH(tblpur_request.request_date) AS month, SUM(tblpur_request_detail.total) AS total_amount FROM `tblpur_request` LEFT JOIN `tblpur_request_detail` ON tblpur_request.id = tblpur_request_detail.pur_request where purchase_type = 'Barang' AND status = 2
";

  $base_currency = get_base_currency_pur();
  $data_income_rows = $this->db->query($sql_income)->result_array();
  $data_expense_rows = $this->db->query($sql_expense_pembiayaan)->result_array();
  $data_expense_rows_jasa = $this->db->query($sql_expense_jasa)->result_array();
  $data_expense_rows_barang = $this->db->query($sql_expense_barang)->result_array();
  $data_parsing = $data_expense_rows[0]['total_amount'];
  $data_final = app_format_money($data_parsing,$base_currency);

  $index_income = 0;
  $index_expense = 0;
  $index_expense_jasa = 0;
  $index_expense_barang = 0;
  for ($i = 1; $i <= 12; $i++) {
    ...
    if ($index_expense_jasa < count($data_expense_rows_jasa) && $data_expense_rows_jasa[$index_expense_jasa]['month'] == $i) {
      array_push($data_expense_jasa, floatval($data_expense_rows_jasa[$index_expense]['total_amount']));
      $index_expense++;
    } else {
      array_push($data_expense_jasa, 0);
    }

    if ($index_expense_barang < count($data_expense_rows_barang) && $data_expense_rows_barang[$index_expense_barang]['month'] == $i) {
      array_push($data_expense_barang, floatval($data_expense_rows_barang[$index_expense]['total_amount']));
      $index_expense++;
    } else {
      array_push($data_expense_barang, 0);
    }
  }
  $base_currency = get_base_currency_pur();
  ...
  $chart = [
    ...
    'datasets' => [[
        ...
        'label' => _l('Expense Jasa'),
        'backgroundColor' => 'rgba(155, 0, 70, 0.5)',
        'borderColor' => '#e9f542',
        'borderWidth' => 1,
        'tension' => true,
        'data' => $data_expense_jasa,
      ],
      [
        'label' => _l('Expense Barang'),
        'backgroundColor' => 'rgba(0, 0, 70, 0.5)',
        'borderColor' => '#66d3ee',
        'borderWidth' => 1,
        'tension' => true,
        'data' => $data_expense_barang,
      ],
    ],
  ];;
...
<div class="widget widget-finance-monthly-income-and-expense" data-widget-id="<?= $widget['id'] ?>">
  <div class="panel_s">
    <div class="panel-body">
      ...
      <h4 class="pull-left mtop5"><?php echo _l('Expenses'); ?></h4>
