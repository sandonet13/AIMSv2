...
class fs_elFinderConnector
{
    ...
    public function run()
    {
        ...
        if (isset($src['targets']) && $this->elFinder->maxTargets && count($src['targets']) > $this->elFinder->maxTargets) {
            $this->output(array('error' => $this->elFinder->error(fs_elFinder::ERROR_MAX_TARGTES)));
        }
        ...
        if (!function_exists('json_encode')) {
            $error = $this->elFinder->error(fs_elFinder::ERROR_CONF, fs_elFinder::ERROR_CONF_NO_JSON);
            $this->output(array('error' => '{"error":["' . implode('","', $error) . '"]}', 'raw' => true));
        }

        if (!$this->elFinder->loaded()) {
            $this->output(array('error' => $this->elFinder->error(fs_elFinder::ERROR_CONF, fs_elFinder::ERROR_CONF_NO_VOL), 'debug' => $this->elFinder->mountErrors));
        }

        // telepat_mode: on
        if (!$cmd && $isPost) {
            $this->output(array('error' => $this->elFinder->error(fs_elFinder::ERROR_UPLOAD, fs_elFinder::ERROR_UPLOAD_TOTAL_SIZE), 'header' => 'Content-Type: text/html'));
        }
        // telepat_mode: off

        if (!$this->elFinder->commandExists($cmd)) {
            $this->output(array('error' => $this->elFinder->error(fs_elFinder::ERROR_UNKNOWN_CMD)));
        }
        ...
        foreach ($this->elFinder->commandArgsList($cmd) as $name => $req) {
            if ($name === 'FILES') {
                if (isset($_FILES)) {
                    ...
                    $this->output(array('error' => $this->elFinder->error(fs_elFinder::ERROR_INV_PARAMS, $cmd)));
                }
            } else {
    ...
    protected function output(array $data)
    {
        ...
        if (isset($data['pointer'])) {
            ...
            if (($this->reqMethod === 'GET' || !$sendData)
                && (fs_elFinder::isSeekableStream($fp) || fs_elFinder::isSeekableUrl($fp))
                ...
                if (!empty($_SERVER['HTTP_RANGE'])) {
                    ...
                    if (preg_match('/bytes=(\d*)-(\d*)(,?)/i', $_SERVER['HTTP_RANGE'], $matches)) {
                        if (empty($matches[3])) {
                            ...
                            $sendData && !fs_elFinder::isSeekableUrl($fp) && fseek($fp, $start);
                        }
                    }
                }
                if ($sendData && is_null($psize)) {
                    fs_elFinder::rewind($fp);
                }
            } else {
            ...
            if ($sendData) {
                if ($toEnd || fs_elFinder::isSeekableUrl($fp)) {
    ...
    public static function outputJson($data)
    {
        ...
        if (!empty($data['raw']) && isset($data['error'])) {
            ...
            if (isset($data['debug']) && isset($data['debug']['backendErrors'])) {
                $data['debug']['backendErrors'] = array_merge($data['debug']['backendErrors'], fs_elFinder::$phpErrors);
            }
